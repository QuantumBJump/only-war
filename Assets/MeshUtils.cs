...
public static class MeshUtils {
	...
	public static Mesh AddToMesh(Mesh mesh, Vector3 pos, float rot, Vector3 baseSize, Vector2 uv00, Vector2 uv11) {
		...
		if (skewed) {
			...
			Vector3 v1 = pos + GetQuaternionEuler(rot - 270) * baseSize;
			Vector3 v2 = pos + GetQuaternionEuler(rot - 180) * baseSize;
			Vector3 v3 = pos + GetQuaternionEuler(rot - 90) * baseSize;
			Vector3 v4 = pos + GetQuaternionEuler(rot - 0) * baseSize;
	...
	public static void AddToMeshArrays(Vector3[] vertices, Vector2[] uvs, int[] triangles, int index, Vector3 pos, float rot, Vector3 baseSize, Vector2 uv00, Vector2 uv11) {
		...
		Vector3 correction = new Vector3(0, 0.005f);
		if (skewed) {
			vertices[vIndex0] = pos + GetQuaternionEuler(rot) * new Vector3(-baseSize.x, baseSize.z) + correction;
			vertices[vIndex1] = pos + GetQuaternionEuler(rot) * new Vector3(-baseSize.x, -baseSize.z) + correction;
			vertices[vIndex2] = pos + GetQuaternionEuler(rot) * new Vector3(baseSize.x, -baseSize.z) + correction;
			vertices[vIndex3] = pos + GetQuaternionEuler(rot) * baseSize + correction;
		} else {
			vertices[vIndex0] = pos + GetQuaternionEuler(rot - 270) * baseSize + correction;
			vertices[vIndex1] = pos + GetQuaternionEuler(rot - 180) * baseSize + correction;
			vertices[vIndex2] = pos + GetQuaternionEuler(rot - 90) * baseSize + correction;
			vertices[vIndex3] = pos + GetQuaternionEuler(rot - 0) * baseSize + correction;
		}
